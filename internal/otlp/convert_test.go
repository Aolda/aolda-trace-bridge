...
func TestConvertAddsOpenStackRequestIDAttribute(t *testing.T) {
	...
	if got := attr(target.Attributes, "openstack.request_id"); got != "d33f246f-86a2-42cc-aae2-dbc5f49a6ff4" {
		t.Fatalf("openstack.request_id = %q", got)
	}
	if got := attr(target.Attributes, "openstack.request_id_prefixed"); got != "req-d33f246f-86a2-42cc-aae2-dbc5f49a6ff4" {
		t.Fatalf("openstack.request_id_prefixed = %q", got)
	}
}

func TestConvertAddsBareOpenStackRequestIDAttribute(t *testing.T) {
	rep := report.Report{
		Info: map[string]any{"name": "total", "started": float64(0), "finished": float64(1)},
		Children: []report.Node{
			{
				...
				Info: map[string]any{
					...
					"meta.raw_payload.wsgi-start": map[string]any{
						"timestamp":  "2026-04-25T15:30:00.000000",
						"request_id": "19d2242a-f77a-4098-a183-adb30d6d9b58",
						"info": map[string]any{
							"request": map[string]any{
								"method": "GET",
								"path":   "/",
